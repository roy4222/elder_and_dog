# Gemini VLM 節點開發指南（Plan B 備案方案）

**⚠️ 警告：本方案為 Plan B 備案，主力請使用 [COCO VLM 開發指南](./coco_vlm_development.md)**

**套件名稱：** `vision_vlm`
**主要節點：** `gemini_vlm_node`
**方案定位：** Plan B 備案（根據 2025/11/19 會議決議）
**開發週次：** W6-W7（若 COCO 方案失敗）
**難度：** ⭐⭐⭐⭐ 中高
...
根據 **2025/11/19 會議決議**，Gemini Robotics API 被調整為備案方案，原因如下：

### 主要風險
1. **審核狀態不確定**：Gemini Robotics API 仍在審核中，無法保證能在 12/17 前取得 API Key
2. **網路依賴**：需要持續網路連線，不適合實機展示場景
3. **API 額度限制**：可能影響開發與測試頻率
4. **延遲不可控**：API 回應時間受網路影響，難以保證 < 2 秒

### 使用時機
僅在以下情況使用本方案：
- ✅ COCO 模型無法識別特定物品（如眼鏡、鑰匙）
- ✅ 需要零樣本識別新類別物品
- ✅ 12/17 發表時使用預錄影片（非實機展示）
