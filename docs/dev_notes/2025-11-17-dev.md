...
### 3. WebRTC 連接故障診斷與根本原因推論（重要進展）

**問題描述**：
- go2_driver_node 啟動時視訊串流正常（"Video frame received"），but WebRTC data channel 停留在 connecting 狀態
- TEST.sh 的 sit/stand/forward 等指令經由 `/webrtc_req` 發出，但機器人無反應
- **觀察現象**：on_data_channel_open() 從未被觸發，導致 data channel 無法建立通訊
...
2. **CycloneDDS 測試結果**
   - 驅動以 `Connection mode: multi` 啟動（非預期的多機器人模式）
   - CycloneDDS 啟動無 WebRTC 相關錯誤，但尚未驗證能穩定控制機器人

3. **WebRTC 握手完整分析**
   - 啟用 DEBUG 級別日誌，添加 ICE/DTLS/SCTP 事件監聽
   - 追蹤握手四階段：
     - ✅ **Phase 1: SDP 交換** → HTTP 加密通道成功，機器人收到 Offer 並回應 Answer
     - ✅ **Phase 2: ICE 連接** → Candidates 交換成功，connection state: completed，UDP 路徑可用
     - ✅ **Phase 3: DTLS 握手** → Connection state: connected，SRTP_AES128_CM_SHA1_80 協商成功
     - ❌ **Phase 4: SCTP 握手** → RTCSctpTransport 發送 InitChunk，機器人未回應

4. **SCTP 握手失敗的觀察**
   - aiortc 發送 `InitChunk(flags=0)` 給機器人
   - T1 超時機制依次觸發：expired 1, 2, 3, 4, 5, 6, 7, 8, 9
   - 機器人端無回應（未返回 InitChunk_ACK）
   - 30 秒後超時，Data channel 終止 (State: closed)
   - 推論：握手卡在 SCTP 協議層，而非 ICE/DTLS 層
...
| 階段 | 狀態 | 觀察 |
|------|------|------|
| **SDP 交換** | ✅ 正常 | HTTP 加密通訊成功，機器人收到 Offer 並回應 Answer |
| **ICE 連線** | ✅ 正常 | Candidates 交換成功，connection state: completed，UDP 路徑可用 |
| **DTLS 握手** | ✅ 正常 | Connection state: connected，SRTP_AES128_CM_SHA1_80 協商完成 |
| **SCTP 握手** | ❌ 失敗 | aiortc 發送 InitChunk，機器人無回應，T1 timeout × 9，data channel 無法開啟 |

**根本原因推論**：
握手流程卡在 Phase 4（SCTP 協議層），機器人端未回應 SCTP InitChunk。可能的原因包括：
1. Go2 韌體的 WebRTC 實現限制（SCTP 不支援或被禁用）
2. WSL2 環境中的 SCTP/NAT 行為異常
3. aiortc/libusrsctp 版本與 Go2 實作不相容
4. go2 專用的 aioice 分支（external_lib/aioice@go2）在 WSL2 環境下表現異常

**已完成的診斷改進**：
- ✅ 環境清理（清除孤立進程、ROS daemon 重啟）→ 無改善
- ✅ 啟用 DEBUG 級日誌 → 現在能看到握手詳情
  - 修改檔案：[go2_driver_node.py:31-36](go2_robot_sdk/go2_robot_sdk/presentation/go2_driver_node.py#L31-L36)
  - 變更：logging.basicConfig(level=logging.DEBUG)
  - 結果：揭示了 SCTP InitChunk timeout × 9 的症狀

- ✅ 添加 STUN 伺服器配置
  - 修改檔案：[go2_connection.py:46-53](go2_robot_sdk/go2_robot_sdk/infrastructure/webrtc/go2_connection.py#L46-L53)
  - 變更：RTCConfiguration + Google STUN servers
  - 結果：ICE/DTLS 連線已正常建立，但 SCTP 握手仍失敗

- ✅ 添加 SCTP 握手監控
  - 修改檔案：[go2_connection.py:110-112, 382-425](go2_robot_sdk/go2_robot_sdk/infrastructure/webrtc/go2_connection.py#L382-L425)
  - 新增：`on_data_channel_created()` 事件監聽、`_monitor_sctp_handshake()` 方法
  - 結果：30 秒超時時輸出詳細診斷（data channel state、connection state、ICE state）

## 🎉 WebRTC 連接問題 - 根本原因已確認並修復！

**最終診斷**：
問題不在 Go2 固件或 WSL2 環境，而是 **aiortc 版本不匹配 + STUN 配置衝突**。

### 版本差異導致問題

| 項目 | 失敗環境 | 成功環境 |
|------|---------|---------|
| aiortc | **1.14.0**（實際安裝） | **1.9.0**（requirements 指定） |
| STUN 配置 | **有（RTCConfiguration）** | **無（預設 RTCPeerConnection）** |
| SCTP 握手 | InitChunk timeout × 9（30+ 秒） | **成功（0.5 秒）** |
...
2. **移除 STUN 配置**
   - 變更：`RTCPeerConnection()` 而非 `RTCPeerConnection(configuration=RTCConfiguration(...))`
   - 原因：在同一 LAN（192.168.x.x ↔ 192.168.x.x）内，STUN 非必需；某些版本的 aiortc 在 STUN 時有 SCTP 相容性問題

### 驗證結果

✅ **連接成功**：Robot validation successful
✅ **Data channel 開啟**：SCTP 握手在 0.5 秒完成
✅ **命令執行**：Stand 指令測試
  - Body height: 0.313 m (lie) → 0.326 m (stand) ✓
  - Error code: 100 → 1002 (standing mode) ✓
✅ **感測器數據流**：持續接收 sportmodestate、robot_pose 等資料

### 關鍵發現

「之前成功連接」的環境很可能就是用 aiortc 1.9.0 且沒有 STUN 配置。
本次失敗是因為：
- pip 安裝依賴時自動升級 aiortc 到 1.14.0（可能來自 torch/torchvision 的間接依賴）
- 我們添加的 STUN 配置在新版 aiortc 中有相容性問題
- Go2 固件本身沒有問題，只是版本配合不当

**建議的後續行動**（優先順序）**：

1. **確認 Go2 韌體版本**（⭐⭐⭐⭐⭐）
   - 目的：縮小 root cause 範圍（固件限制 vs. 環境問題）
   - 操作：透過手機 App / Web 界面查詢機器人當前韌體版本
   - 後續：檢查 RoboVerse Issues/Discussions 中是否有該版本的 SCTP/WebRTC 相關問題

2. **在原生 Linux 環境測試**（⭐⭐⭐⭐）
   - 目的：排除 WSL2 SCTP/NAT 限制的影響
   - 操作：在非 WSL2 的 Ubuntu 虛擬機上執行同一驅動，記錄握手過程
   - 預期：若原生 Linux 也失敗，問題更可能是固件相關；若成功，則是 WSL2 相關

3. **檢查 aioice@go2 子模組**（⭐⭐⭐）
   - 目的：確認是否有 WSL2/NAT 相關修補
   - 操作：
     ```bash
...
5. **考慮備用方案**（⭐⭐）
   - 直接使用 Go2 HTTP REST API 而非 WebRTC（如果有公開文件）
   - 或聯絡 Unitree/RoboVerse 社群取得機器人端 WebRTC 日誌
