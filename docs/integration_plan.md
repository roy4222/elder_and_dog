...
**B. Gemini API 準備**
- [ ] 確認 Gemini Robotics API 申請狀態
- [ ] 取得 API Key（或使用測試額度）
- [ ] 測試 API 呼叫（Python 腳本驗證）
...
# 目錄結構（詳見 package_structure.md）
vision_vlm/
├── vision_vlm/
│   ├── __init__.py
│   ├── gemini_vlm_node.py        # 主節點（本週完成框架）
│   ├── gemini_api_client.py      # API 包裝類
...
| 風險 | 影響 | 機率 | Plan A | Plan B | Plan C |
|------|------|------|--------|--------|--------|
| Gemini API 不穩定 | 🔴 高 | 中 | 快取 + 降頻 | 預錄結果 | COCO 替代 |
...
- [Gemini VLM 開發指南](./gemini_vlm_development.md)
