...
**修訂日期：** 2025年11月20日（依 2025/11/19 會議決議同步）
**專題目標：** 結合機器狗移動平台、ROS2 導航系統及 COCO 本地 VLM（Gemini 為備案），開發一個具備自主導航、環境感知和指定物品尋獲能力的智慧尋物系統。  
> 註：本文件內容已依 2025/11/19 會議決議驗證，所有時程/策略與 `conformance_check_plan.md` 對應。
...
| **W6** | **COCO VLM 整合（Plan A）** | [✅] 部署 TorchVision COCO 模型、完成 `/detected_objects` 節點與 GPU 推論測試；同步追蹤 Gemini API 狀態（Plan B）。 |
